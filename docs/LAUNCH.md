...
| 集成测试 | 后端 27 例（注册登录鉴权/会话生命周期/心情成就/限流器/Redis 存储/会员闭环），`mvn test` 全绿；跑测试需本地 pgvector（建 `ai_love_test` 库 + vector 扩展） |
| 前端组件测试 | 37 例（markdown XSS 净化/api 401 与 SSE 错误帧/auth 存储/协议页/登录表单/设置弹窗/会员闭环），`npm test` 全绿 |
