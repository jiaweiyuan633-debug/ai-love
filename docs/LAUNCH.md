...
- 内存限流在 FC 多实例下各实例独立计数，防刷能力打折；防爆破仍有效（单 IP 连续尝试会被 429 挡住的概率高）。多实例部署可设 `RATE_LIMIT_BACKEND=redis` 切换共享计数（见 docs/DEPLOY.md 第七节）。
- 体验模式下会员状态/订单/每日额度计数都在进程内存中：重启或多实例不共享、重新部署清零（与对话数据同级别局限）。接 RDS 后（`PERSISTENCE_ENABLED=true`）自动落库，跨重启保留。
...
| 接口限流 | 登录/注册按 IP 每分钟 10 次防爆破；AI 消耗端点按用户每分钟 30 次防刷（`RateLimitFilter` + `RateLimitStore` 抽象：默认内存滑窗，`RATE_LIMIT_BACKEND=redis` 切换 Redis 共享计数、故障自动放行） |
...
| 集成测试 | 后端 35 例（注册登录鉴权/会话生命周期/心情成就/限流器/Redis 存储/会员闭环/注销与找回密码/CORS），`mvn test` 全绿；跑测试需本地 pgvector（建 `ai_love_test` 库 + vector 扩展） |
| 前端组件测试 | 43 例（markdown XSS 净化/api 401 与 SSE 错误帧/auth 存储/协议页/登录表单与找回密码/设置弹窗/会员闭环/邮箱绑定/注销确认），`npm test` 全绿 |
| CI 流水线 | GitHub Actions 三 job：后端测试（pgvector 服务容器自动建扩展）、前端测试+构建、MCP 编译；README 挂 badge |
| Redis 分布式限流 | `RATE_LIMIT_BACKEND=redis` 切换多实例共享计数（Lua 固定窗口，故障 fail-open）；默认 memory 不依赖 Redis，启用步骤见 docs/DEPLOY.md 第七节 |
| 会员订阅 | 免费每日 20 条 AI 对话（`MEMBERSHIP_DAILY_FREE_LIMIT` 可调），VIP 不限 + AI 限流翻倍 + 徽章；月/季/年三档下单 + 模拟支付回调（幂等，续费顺延）；前端设置弹窗"💎 会员" tab + 聊天页 VIP 徽章。真实微信/支付宝需企业资质 + ICP 备案，届时仅替换支付回调入口 |
| 账号管理 | 一键注销（DELETE /auth/me，级联删除会话/消息/记忆/心情/朋友圈/情侣/会员数据并清理进程内上下文缓存）；邮箱绑定 + 6 位验证码自助找回密码（验证码只存 SHA-256、10 分钟有效、5 次尝试上限、重发覆盖旧码；未配置 SMTP 时接口 503 优雅降级、前端隐藏入口；找回请求纳入登录防爆破限流桶） |
| 原生客户端 | Tauri 2 打包：打 `v*` tag 自动构建 Windows NSIS/MSI + macOS dmg（GitHub Release）+ Android 调试签名 APK（Actions 构件）；前端 `VITE_API_BASE` 指向云端、后端 CORS 放行 tauri 壳来源，账号与数据同网页版互通。详见 [docs/TAURI.md](TAURI.md) |

## 明确未做（后续方向）

- 真实支付网关（微信/支付宝需企业资质 + ICP 备案域名；模拟支付闭环已就绪，接入时仅替换回调入口）、多语言；
- 手机号注册/实名（对外运营规模化后按需）。
