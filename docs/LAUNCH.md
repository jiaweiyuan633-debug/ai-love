...
| 集成测试 | 后端 18 例（注册登录鉴权/会话生命周期/心情成就/限流器/Redis 存储），`mvn test` 全绿；跑测试需本地 pgvector（建 `ai_love_test` 库 + vector 扩展） |
| 前端组件测试 | 35 例（markdown XSS 净化/api 401 与 SSE 错误帧/auth 存储/协议页/登录表单/设置弹窗），`npm test` 全绿 |
| CI 流水线 | GitHub Actions 三 job：后端测试（pgvector 服务容器自动建扩展）、前端测试+构建、MCP 编译；README 挂 badge |

## 明确未做（后续方向）

- 支付/订阅（真实微信/支付宝需企业资质 + ICP 备案，先做模拟支付闭环）、多语言；
- 手机号注册/实名（对外运营规模化后按需）。
