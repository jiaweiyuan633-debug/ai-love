...
| ✅ 测试 | 后端 `mvn test` 27 例（鉴权/会话/心情成就/限流器/Redis 存储/会员闭环）+ 前端 `npm test` 37 例（XSS 净化/401 与 SSE 错误帧/表单/弹窗），CI 自动运行；本地跑测试需 pgvector（建 `ai_love_test` 库 + vector 扩展） |

## 全流程

需求分析 → 架构设计 → 开发实现 → 产品化升级（用户系统/持久化/记忆/语音/主题）→ 竞品吸收优化（角色化/朋友圈/心情/成就）→ 上线冲刺加固（安全/合规/测试）→ 工程化与商业化（CI/前端测试/Redis 限流/会员订阅）→ 部署上线（详见 `docs/`）
